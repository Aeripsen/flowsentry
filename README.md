...
- The testing did not stop at the paper's architecture: a tuned **XGBoost** arm beats the shipped
  forest on the headline PR-AUC (**0.9852 vs 0.9767**) and roughly doubles full-coverage recall on
  the rare families, while losing full-coverage accuracy (0.7533 vs 0.8317). Both sides of that
  trade are committed in `artifacts/gbdt_comparison.json`, with SHAP attribution for the winner.
  See [boosted trees](#boosted-trees-against-the-forest-and-shap-attribution).
...
### Boosted trees against the forest, and SHAP attribution

The shipped model is the paper's two-stage forest and that stays the reference point; the paper's
result stands as published. This repo keeps measuring past it, and after the hierarchy ablation the
next question was the model family itself. `python scripts/gbdt_comparison.py` (needs
`pip install -e ".[gbdt]"`) runs LightGBM and XGBoost on the same 132 features, the same grouped
leakage-safe split and the same metrics battery, with the whole 8-config grid printed per family so
nothing is searched silently, and the winners refit on the full training split.
`artifacts/gbdt_comparison.json` holds every number below.

| Arm | Binary PR-AUC | Accuracy | Macro-F1 | ECE (raw) |
|---|---|---|---|---|
| two-stage forest (shipped) | 0.9767 | **0.8317** | **0.3911** | **0.0362** |
| lightgbm (lr 0.05, 200 iter, 31 leaves) | 0.9837 | 0.7973 | 0.3825 | 0.0559 |
| xgboost (lr 0.05, 200 iter, depth 6) | **0.9852** | 0.7533 | 0.3487 | 0.0399 |

Both boosted arms beat the forest on the headline ranking metric (XGBoost by 0.0085 PR-AUC) and
roughly double full-coverage recall on the rare families: UDP-OVH 0.2545 vs the forest's 0.0182,
UDP-GAME 0.2500 vs 0.1154, UDP-bypass-v1 0.4483 vs 0.2931 (XGBoost numbers; LightGBM is close).
They pay for it in hard labels: accuracy and macro-F1 drop, and benign recall falls to 0.5693
(XGBoost) / 0.6635 (LightGBM) against the forest's 0.7481, so the boosted arms buy ranking and
rare-family coverage by spending benign precision. Read it as a division of labour: if flows are
ranked for triage, XGBoost is the better scorer on this sample; if every flow must get a hard label
at full coverage, the shipped forest is still ahead. The reject knob tells the same story from the
other end: at threshold 0.99 LightGBM answers 66.9% of flows at 0.9964 reliability against the
forest's 64.8% at 0.9932, while XGBoost answers 59.1% at 0.9982.

Raw boosted confidence is a little worse calibrated than the forest's; an isotonic map, fit and
picked on separate held-out carves (Platt lost the pick for both arms), repairs the number's
meaning: test ECE 0.0397 to 0.0174 for XGBoost and 0.0625 to 0.0150 for LightGBM, measured on the
validation-fit models for the same reason the earlier calibration experiment refit its own model.

`python scripts/shap_attribution.py` then commits what the winner actually looks at: exact
TreeExplainer values over the full test split, `artifacts/shap_top_features.json` (top 15 features
by mean |SHAP|, with the per-class breakdown) and `artifacts/shap_summary.png`. The top of the
list reads as network sense rather than artifact hunting: `fwd_bwd_byte_ratio`, `min_pkt`,
`pkt_size_entropy`, `delta_pay_size_min`, `median_iat`. It explains the single-stage winner, not
the two-stage forest, because the escalation gate means the hierarchy has no single honest SHAP
story; the script says so in its own docstring.
